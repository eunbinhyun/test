...
public class CubemanController : MonoBehaviour 
{
    ...
    //함수추가
    void abcd() {
        Destroy(GameObject.FindWithTag("etag"), 0.0f); //etag 태그를 가진 겜오브젝트를 지워라

        //    Debug.Log("hello"); //포지션을 각각 x,y,z축 값을 지정함
		...
		for(int i = 0; i < bones.Length; i++) 
		{
			if(bones[i] != null)
			{
				...
				if(manager.IsJointTracked(playerID, joint))
				{
                    ...
                    ////수정해보기///(박스 위치에 손(i번째 뼈 transform.localPosition)이 닿으면 없어지게)
                    /*if (box.transform.position.x+0.5 >= bones[7].transform.position.x && box.transform.position.x - 0.5 <= bones[7].transform.position.x)
                    {
                        if (box.transform.position.y + 0.5 >= bones[7].transform.position.y && box.transform.position.y - 0.5 <= bones[7].transform.position.y) {
                            Debug.Log("cab");
                        }
                    }*/

                    //수정0211//
                    box.transform.position = bones[7].transform.position;
                    //이때 변화된것은 사람이 공중에 떠있음
                    //수정전:     box.transform.localPosition = bones[7].transform.localPosition;
                    //이거는 좌우가 반대로감
                    //0211끝//


                    // Debug.Log(bones[7].transform.localPosition); //오른손의 좌표 출력
                    //오른팔을 들면 손의 y 좌표가 0.8~1이다
                    if (bones[7].transform.localPosition.y >= 0.7f )
                    {
                       abcd();
